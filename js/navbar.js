...
function createCSSClassesToAdd(c1, c2) {
    ...
    if (res.length > 3) {
        ...
        md_class.push('bg-' + res[1] + '-50');
    }
    for (i = 1; i <= 9; i++) {
        if (res.length > 3) {
            md_class.push('bg-' + res[1] + '-' + res[2] + '-' + i + '00');
        } else {
            md_class.push('bg-' + res[1] + '-' + i + '00');
        }
    }
    console.log(md_class)
    if (res[1] == 'brown' || res[2] == 'gray' || res[1] == 'gray') {} else {
        console.log(res.length)
        if (res.length > 3) {

            md_class.push('bg-' + res[1] + '-' + res[2] + '-A100');
            md_class.push('bg-' + res[1] + '-' + res[2] + '-A200');
            md_class.push('bg-' + res[1] + '-' + res[2] + '-A400');
            md_class.push('bg-' + res[1] + '-' + res[2] + '-A700');
        } else {
            md_class.push('bg-' + res[1] + '-A100');
            md_class.push('bg-' + res[1] + '-A200');
            md_class.push('bg-' + res[1] + '-A400');
            md_class.push('bg-' + res[1] + '-A700');
        }
    }
...
function createButton(data) {
    ...
    data.forEach((element) => {
        ...
        nav.innerHTML = str[0];
        nav.classList.add(str[0]);
        nav.classList.add('navbar');
        nav.classList.add('navbar-expand-lg');
        nav.classList.add('mr-5');
        nav.classList.add('mb-3');
        nav.classList.add('mt-3');
        buttonDiv.appendChild(nav).style.height = '5rem';
    });

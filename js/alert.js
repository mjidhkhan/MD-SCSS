...
function createCSSClassesToAdd(c1, c2) {
    ...
    //console.log(res);
    if (res.length > 3) {
        md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-50');
    } else {
        md_class.push('alert ' + 'alert-' + res[1] + '-50');
    }
    for (i = 1; i <= 9; i++) {
        if (res.length > 3) {
            md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-' + i + '00');
        } else {
            md_class.push('alert ' + 'alert-' + res[1] + '-' + i + '00');
        }
    }
    if (res[1] == 'brown' || res[2] == 'gray' || res[1] == 'gray') {} else {
        if (res.length > 3) {
            md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-A100');
            md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-A200');
            md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-A400');
            md_class.push('alert ' + 'alert-' + res[1] + '-' + res[2] + '-A700');
        } else {
            md_class.push('alert ' + 'alert-' + res[1] + '-A100');
            md_class.push('alert ' + 'alert-' + res[1] + '-A200');
            md_class.push('alert ' + 'alert-' + res[1] + '-A400');
            md_class.push('alert ' + 'alert-' + res[1] + '-A700');
        }
    }
...
function createButton(data) {
    ...
    data.forEach((element) => {
        ...
        buttonDiv.appendChild(button).style.height = '5rem';
    });
